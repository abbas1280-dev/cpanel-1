...
export function getDefaultZoneRecords(domain: string, serverIp: string) {
  return [
    { id: 'rec_soa', domain, name: domain, type: 'SOA', ttl: 86400, record: 'ns1.hoster1280.shop. hostmaster.hoster1280.shop.' },
    { id: 'rec_ns1', domain, name: domain, type: 'NS', ttl: 86400, record: 'ns1.hoster1280.shop' },
    { id: 'rec_ns2', domain, name: domain, type: 'NS', ttl: 86400, record: 'ns2.hoster1280.shop' },
    { id: 'rec_a1', domain, name: domain, type: 'A', ttl: 14400, record: serverIp },
    { id: 'rec_cname_www', domain, name: `www.${domain}`, type: 'CNAME', ttl: 14400, record: domain },
    { id: 'rec_a_cpanel', domain, name: `cpanel.${domain}`, type: 'A', ttl: 14400, record: serverIp },
    { id: 'rec_a_mail', domain, name: `mail.${domain}`, type: 'A', ttl: 14400, record: serverIp },
    { id: 'rec_a_ftp', domain, name: `ftp.${domain}`, type: 'A', ttl: 14400, record: serverIp },
    { id: 'rec_mx', domain, name: domain, type: 'MX', ttl: 14400, record: `mail.${domain}`, priority: 10 },
    { id: 'rec_spf', domain, name: domain, type: 'TXT', ttl: 14400, record: `v=spf1 a mx ip4:${serverIp} ~all` }
  ];
}
...
<body>
  <div class="card">
    ...
    let redirectsSnippet = '';
    try {
      const redFile = path.join(domainRoot, 'redirects.json');
      if (fs.existsSync(redFile)) {
        const reds = JSON.parse(fs.readFileSync(redFile, 'utf-8'));
        const matching = reds.filter((r: any) => r.domain.toLowerCase() === domain.toLowerCase() && r.status !== 'inactive');
        for (const r of matching) {
          const code = r.type === 'permanent' ? '301' : '302';
          let dir = (r.directory || '/').trim();
          if (!dir.startsWith('/')) dir = `/${dir}`;
          const target = r.redirectUrl.trim();
          if (dir === '/') {
            redirectsSnippet += `
...
`;
            ...
            redirectsSnippet += `
    # Custom Redirect: ${cleanDir}
    location = ${cleanDir} {
        return ${code} ${target};
    }
    location ${cleanDir}/ {
        return ${code} ${target.replace(/\/+$/, '')}$request_uri;
    }
`;
          }
        }
...
@ IN TXT "v=spf1 a mx ip4:${cleanIp} ~all"
`;
...
export async function syncBind9ZoneWithRecords(domain: string, records: any[], serverIp?: string) {
  let cleanIp = serverIp || getBestIp();
  if (!cleanIp || cleanIp.startsWith('169.254.') || cleanIp.startsWith('127.')) {
    cleanIp = '208.72.218.129';
  }
  ...
  if (process.platform === 'linux' && fs.existsSync(zoneFile)) {
    try {
      const existing = fs.readFileSync(zoneFile, 'utf-8');
      const m = existing.match(/(\d{8})(\d{2})/);
      if (m && m[1] === datePrefix) {
        counter = parseInt(m[2], 10) + 1;
      }
    } catch (e) {}
  }
  const serial = `${datePrefix}${String(counter).padStart(2, '0')}`;

  let zoneContent = `; Authoritative zone for ${domain}
; Auto-generated by HOSTER 1280 Advanced Zone Editor
$TTL 86400
@ IN SOA ns1.hoster1280.shop. hostmaster.hoster1280.shop. (
    ${serial} ; Serial
    7200      ; Refresh
...
`;
  ...
  for (const rec of records) {
    if (!rec || rec.type === 'SOA' || rec.type === 'NS') continue;
    let rName = (rec.name || '@').trim();
    if (rName === domain || rName === `${domain}.`) rName = '@';
    else if (rName.endsWith(`.${domain}`)) rName = rName.slice(0, -(domain.length + 1));
    else if (rName.endsWith(`.${domain}.`)) rName = rName.slice(0, -(domain.length + 2));

    const ttl = rec.ttl || 14400;
    const type = (rec.type || 'A').toUpperCase();
    const val = (rec.record || rec.value || '').trim();

    if (type === 'MX') {
      const priority = rec.priority || 10;
      const target = val.endsWith('.') ? val : `${val}.`;
      zoneContent += `${rName} ${ttl} IN MX ${priority} ${target}\n`;
    } else if (type === 'TXT') {
      const cleanTxt = val.startsWith('"') && val.endsWith('"') ? val : `"${val.replace(/"/g, '\\"')}"`;
      zoneContent += `${rName} ${ttl} IN TXT ${cleanTxt}\n`;
    } else if (type === 'CNAME') {
      const target = val.endsWith('.') ? val : (val.includes('.') ? `${val}.` : val);
      zoneContent += `${rName} ${ttl} IN CNAME ${target}\n`;
    } else if (type === 'SRV') {
      const prio = rec.priority || 0;
      const weight = rec.weight || 0;
      const port = rec.port || 80;
      const target = val.endsWith('.') ? val : `${val}.`;
      zoneContent += `${rName} ${ttl} IN SRV ${prio} ${weight} ${port} ${target}\n`;
    } else {
      zoneContent += `${rName} ${ttl} IN ${type} ${val}\n`;
    }
  }
  ...
  if (process.platform === 'linux') {
    try {
      ...
      console.warn('Bind9 zone sync with records warning:', err.message);
    }
  }
}
      ...
      server.middlewares.use(async (req, res, next) => {
        ...
        if (url.startsWith('/api/cpanel/domains/create') && request.method === 'POST') {
          try {
            ...
            let finalDomain = '';
            const domainType: 'subdomain' | 'addon' | 'alias' = type === 'alias' ? 'alias' : (type === 'addon' ? 'addon' : 'subdomain');
            ...
            // Requirement #9: Subdomain's document root MUST be OUTSIDE public_html (e.g. /subdomains/blog or /blog.example.com)
            let effectiveDocRoot = '';
            if (domainType === 'subdomain') {
              if (shareDocumentRoot) {
                effectiveDocRoot = '/public_html';
              } else {
                const requested = (documentRoot || `subdomains/${subdomainPrefix || finalDomain.split('.')[0]}`).trim();
                let clean = requested.replace(/^\/+/, '');
                if (clean.startsWith('public_html/')) {
                  clean = `subdomains/${clean.replace('public_html/', '')}`;
                }
                effectiveDocRoot = `/${clean || `subdomains/${finalDomain.split('.')[0]}`}`;
              }
            } else if (domainType === 'alias') {
              const targetParent = (parentDomain || mainDomain).trim().toLowerCase();
              const parentEntry = list.find((d: any) => d.domain.toLowerCase() === targetParent) || list.find((d: any) => d.isMain);
              effectiveDocRoot = parentEntry ? parentEntry.documentRoot : '/public_html';
            } else {
...
`;
        ...
        // =========================================================================
        // 24b. CPANEL DOMAINS: REDIRECTS (301 / 302)
        // =========================================================================
        if (url.startsWith('/api/cpanel/domains/redirects') && request.method === 'GET') {
          try {
            const parsed = new URL('http://localhost' + url);
            const mainDomain = parsed.searchParams.get('domain') || 'turkyhub.com';
            const domainRoot = path.join(STORAGE_ROOT, 'domains', mainDomain);
            ...
            let redirects: any[] = [];
            if (fs.existsSync(redFile)) {
              try { redirects = JSON.parse(fs.readFileSync(redFile, 'utf-8')); } catch (e) {}
            }
            response.setHeader('Content-Type', 'application/json');
            response.end(JSON.stringify({ success: true, redirects }));
          } catch (e) {
            response.statusCode = 500;
            response.end(JSON.stringify({ error: String(e) }));
          }
        ...
        if (url.startsWith('/api/cpanel/domains/redirect/create') && request.method === 'POST') {
          try {
            const body = await parseJsonBody(request);
            const { mainDomain, domain, directory, redirectUrl, type = 'permanent', matchType = 'all' } = body;
            if (!mainDomain || !domain || !redirectUrl) {
              response.statusCode = 400;
              response.end(JSON.stringify({ error: 'mainDomain, domain and redirectUrl are required' }));
              return;
            }

            if (!redirectUrl.startsWith('http://') && !redirectUrl.startsWith('https://')) {
              response.statusCode = 400;
              response.end(JSON.stringify({ error: 'Redirect target URL must begin with http:// or https://' }));
              return;
            }
            ...
            fs.writeFileSync(redFile, JSON.stringify(redirects, null, 2));

            // Sync with live Nginx vhost
            const regFile = getDomainsRegistryFile(domainRoot);
            if (fs.existsSync(regFile)) {
              const list = JSON.parse(fs.readFileSync(regFile, 'utf-8'));
              const target = list.find((d: any) => d.domain.toLowerCase() === domain.toLowerCase());
              if (target) {
                await syncLiveNginxVHost(mainDomain, target.domain, target.documentRoot, !!target.forceHttps);
              }
            }
            ...
            response.setHeader('Content-Type', 'application/json');
            response.end(JSON.stringify({ success: true, redirect: newRedirect }));
          } catch (e) {
            response.statusCode = 500;
            response.end(JSON.stringify({ error: String(e) }));
          }
        ...
        if (url.startsWith('/api/cpanel/domains/redirect/delete') && request.method === 'POST') {
          try {
            const body = await parseJsonBody(request);
            const { mainDomain, id } = body;
            if (!mainDomain || !id) {
              response.statusCode = 400;
              response.end(JSON.stringify({ error: 'mainDomain and id are required' }));
              return;
            }
            ...
            const redFile = path.join(domainRoot, 'redirects.json');
            if (fs.existsSync(redFile)) {
              let redirects = JSON.parse(fs.readFileSync(redFile, 'utf-8'));
              const target = redirects.find((r: any) => r.id === id);
              ...
              if (target) {
                ...
                if (fs.existsSync(regFile)) {
                  const list = JSON.parse(fs.readFileSync(regFile, 'utf-8'));
                  const dItem = list.find((d: any) => d.domain.toLowerCase() === target.domain.toLowerCase());
                  if (dItem) {
                    await syncLiveNginxVHost(mainDomain, dItem.domain, dItem.documentRoot, !!dItem.forceHttps);
                  }
                }
              }
            }
            ...
            response.setHeader('Content-Type', 'application/json');
            response.end(JSON.stringify({ success: true, message: 'Redirect deleted successfully' }));
          } catch (e) {
            response.statusCode = 500;
            response.end(JSON.stringify({ error: String(e) }));
          }
        ...
        // =========================================================================
        // 24c. CPANEL DOMAINS: ADVANCED ZONE EDITOR (Bind9 Zone CRUD)
        // =========================================================================
        if (url.startsWith('/api/cpanel/dns/zone') && request.method === 'GET') {
          try {
            const parsed = new URL('http://localhost' + url);
            const mainDomain = parsed.searchParams.get('domain') || 'turkyhub.com';
            const targetDomain = (parsed.searchParams.get('targetDomain') || mainDomain).trim().toLowerCase();

            const domainRoot = path.join(STORAGE_ROOT, 'domains', mainDomain);
            const recFile = path.join(domainRoot, `dns_records_${targetDomain}.json`);
            let records: any[] = [];
            const serverIp = getBestIp();

            if (fs.existsSync(recFile)) {
              try { records = JSON.parse(fs.readFileSync(recFile, 'utf-8')); } catch (e) {}
            } else {
              records = getDefaultZoneRecords(targetDomain, serverIp);
              fs.writeFileSync(recFile, JSON.stringify(records, null, 2));
              await syncBind9ZoneWithRecords(targetDomain, records, serverIp);
            }

            response.setHeader('Content-Type', 'application/json');
            response.end(JSON.stringify({ success: true, targetDomain, records, serverIp }));
          } catch (e) {
            response.statusCode = 500;
            response.end(JSON.stringify({ error: String(e) }));
          }
        ...
        if (url.startsWith('/api/cpanel/dns/record/create') && request.method === 'POST') {
          try {
            const body = await parseJsonBody(request);
            const { mainDomain, targetDomain, record } = body;
            if (!mainDomain || !targetDomain || !record || !record.name || !record.type || !record.record) {
              response.statusCode = 400;
              response.end(JSON.stringify({ error: 'mainDomain, targetDomain and record (name, type, record) are required' }));
              return;
            }

            const domainRoot = path.join(STORAGE_ROOT, 'domains', mainDomain);
            const recFile = path.join(domainRoot, `dns_records_${targetDomain.toLowerCase()}.json`);
            let records: any[] = [];
            if (fs.existsSync(recFile)) {
              try { records = JSON.parse(fs.readFileSync(recFile, 'utf-8')); } catch (e) {}
            } else {
              records = getDefaultZoneRecords(targetDomain, getBestIp());
            }
            ...
            const newRec = {
              ...
              record: record.record.trim(),
              priority: record.priority !== undefined ? parseInt(record.priority, 10) : undefined,
              weight: record.weight !== undefined ? parseInt(record.weight, 10) : undefined,
              port: record.port !== undefined ? parseInt(record.port, 10) : undefined
            };

            records.push(newRec);
            fs.writeFileSync(recFile, JSON.stringify(records, null, 2));

            await syncBind9ZoneWithRecords(targetDomain, records);

            response.setHeader('Content-Type', 'application/json');
            response.end(JSON.stringify({ success: true, record: newRec }));
          } catch (e) {
            response.statusCode = 500;
            response.end(JSON.stringify({ error: String(e) }));
          }
        ...
        if (url.startsWith('/api/cpanel/dns/record/update') && request.method === 'POST') {
          try {
            const body = await parseJsonBody(request);
            const { mainDomain, targetDomain, record } = body;
            if (!mainDomain || !targetDomain || !record || !record.id) {
              response.statusCode = 400;
              response.end(JSON.stringify({ error: 'mainDomain, targetDomain and record.id are required' }));
              return;
            }
            ...
            records[idx] = {
              ...records[idx],
              name: record.name ? record.name.trim() : records[idx].name,
              type: record.type ? record.type.toUpperCase() : records[idx].type,
              ttl: record.ttl ? parseInt(record.ttl, 10) : records[idx].ttl,
              record: record.record ? record.record.trim() : records[idx].record,
              priority: record.priority !== undefined ? parseInt(record.priority, 10) : records[idx].priority,
              weight: record.weight !== undefined ? parseInt(record.weight, 10) : records[idx].weight,
              port: record.port !== undefined ? parseInt(record.port, 10) : records[idx].port
            };
            ...
            response.setHeader('Content-Type', 'application/json');
            response.end(JSON.stringify({ success: true, record: records[idx] }));
          } catch (e) {
            response.statusCode = 500;
            response.end(JSON.stringify({ error: String(e) }));
          }
        ...
        if (url.startsWith('/api/cpanel/dns/record/delete') && request.method === 'POST') {
          try {
            const body = await parseJsonBody(request);
            const { mainDomain, targetDomain, id } = body;
            if (!mainDomain || !targetDomain || !id) {
              response.statusCode = 400;
              response.end(JSON.stringify({ error: 'mainDomain, targetDomain and id are required' }));
              return;
            }
            ...
            response.setHeader('Content-Type', 'application/json');
            response.end(JSON.stringify({ success: true, message: 'Record deleted successfully' }));
          } catch (e) {
            response.statusCode = 500;
            response.end(JSON.stringify({ error: String(e) }));
          }
        ...
        // =========================================================================
        // 24d. CPANEL DOMAINS: DYNAMIC DNS (DDNS) ENGINE
        // =========================================================================
        if (url.startsWith('/api/cpanel/ddns/tokens') && request.method === 'GET') {
          try {
            const parsed = new URL('http://localhost' + url);
            const mainDomain = parsed.searchParams.get('domain') || 'turkyhub.com';
            const ddnsFile = path.join(STORAGE_ROOT, 'ddns_tokens.json');
            let tokens: any[] = [];
            if (fs.existsSync(ddnsFile)) {
              try { tokens = JSON.parse(fs.readFileSync(ddnsFile, 'utf-8')); } catch (e) {}
            }
            const filtered = tokens.filter((t: any) => t.domain.toLowerCase() === mainDomain.toLowerCase());
            response.setHeader('Content-Type', 'application/json');
            response.end(JSON.stringify({ success: true, tokens: filtered }));
          } catch (e) {
            response.statusCode = 500;
            response.end(JSON.stringify({ error: String(e) }));
          }
        ...
        if (url.startsWith('/api/cpanel/ddns/tokens/generate') && request.method === 'POST') {
          try {
            const body = await parseJsonBody(request);
            const { mainDomain, subdomain, targetDomain } = body;
            if (!mainDomain || !subdomain) {
              response.statusCode = 400;
              response.end(JSON.stringify({ error: 'mainDomain and subdomain are required' }));
              return;
            }

            const cleanSub = subdomain.trim().toLowerCase().replace(/[^a-z0-9-]/g, '');
            const parent = (targetDomain || mainDomain).trim().toLowerCase();
            const fullDomain = `${cleanSub}.${parent}`;
            const token = 'ddns_' + crypto.randomBytes(16).toString('hex');
            const serverIp = getBestIp();
            ...
            fs.writeFileSync(ddnsFile, JSON.stringify(tokens, null, 2));

            // Ensure an A record exists for this subdomain in the DNS zone
            const domainRoot = path.join(STORAGE_ROOT, 'domains', mainDomain);
            const recFile = path.join(domainRoot, `dns_records_${parent}.json`);
            let records: any[] = [];
            if (fs.existsSync(recFile)) {
              try { records = JSON.parse(fs.readFileSync(recFile, 'utf-8')); } catch (e) {}
            } else {
              records = getDefaultZoneRecords(parent, serverIp);
            }

            const existingRec = records.find((r: any) => r.name.toLowerCase() === fullDomain.toLowerCase() && r.type === 'A');
            if (!existingRec) {
              records.push({
                id: 'rec_ddns_' + Date.now(),
                domain: parent,
                name: fullDomain,
                type: 'A',
                ttl: 300,
              ...
              fs.writeFileSync(recFile, JSON.stringify(records, null, 2));
              await syncBind9ZoneWithRecords(parent, records);
            }

            response.setHeader('Content-Type', 'application/json');
            response.end(JSON.stringify({ success: true, tokenItem: newToken }));
          } catch (e) {
            response.statusCode = 500;
            response.end(JSON.stringify({ error: String(e) }));
          }
        ...
        if (url.startsWith('/api/cpanel/ddns/tokens/delete') && request.method === 'POST') {
          try {
            const body = await parseJsonBody(request);
            const { id } = body;
            if (!id) {
            ...
            const ddnsFile = path.join(STORAGE_ROOT, 'ddns_tokens.json');
            if (fs.existsSync(ddnsFile)) {
              let tokens = JSON.parse(fs.readFileSync(ddnsFile, 'utf-8'));
              tokens = tokens.filter((t: any) => t.id !== id);
              fs.writeFileSync(ddnsFile, JSON.stringify(tokens, null, 2));
            }

            response.setHeader('Content-Type', 'application/json');
            response.end(JSON.stringify({ success: true, message: 'DDNS Token revoked' }));
          } catch (e) {
            response.statusCode = 500;
            response.end(JSON.stringify({ error: String(e) }));
          }
        ...
        // Public DDNS update endpoint
        if (url.startsWith('/api/ddns/update') && request.method === 'GET') {
          try {
            const parsed = new URL('http://localhost' + url);
            const reqDomain = (parsed.searchParams.get('domain') || '').trim().toLowerCase();
            const reqToken = (parsed.searchParams.get('token') || '').trim();
            const specifiedIp = (parsed.searchParams.get('ip') || '').trim();

            if (!reqDomain || !reqToken) {
              response.statusCode = 400;
              response.end(JSON.stringify({ error: 'Both domain and token query parameters are required' }));
              return;
            }
            ...
            const tokenItem = tokens.find((t: any) => t.token === reqToken && t.fullDomain.toLowerCase() === reqDomain);
            if (!tokenItem) {
              response.statusCode = 403;
              response.end(JSON.stringify({ error: 'Authentication failed: Invalid DDNS token or domain mismatch' }));
              return;
            }

            let clientIp = specifiedIp;
            if (!clientIp) {
              const xff = request.headers['x-forwarded-for'];
              if (xff) {
                clientIp = (typeof xff === 'string' ? xff : xff[0]).split(',')[0].trim();
              } else {
            ...
            const parentDomain = tokenItem.targetDomain || tokenItem.domain;
            const domainRoot = path.join(STORAGE_ROOT, 'domains', tokenItem.domain);
            const recFile = path.join(domainRoot, `dns_records_${parentDomain}.json`);
            let records: any[] = [];
            if (fs.existsSync(recFile)) {
              try { records = JSON.parse(fs.readFileSync(recFile, 'utf-8')); } catch (e) {}
            }
            ...
            for (const r of records) {
              if (r.name.toLowerCase() === reqDomain && r.type === 'A') {
                r.record = clientIp;
                r.ttl = 300;
                found = true;
              }
            ...
            await syncBind9ZoneWithRecords(parentDomain, records);

            tokenItem.currentIp = clientIp;
            tokenItem.lastUpdated = new Date().toISOString();
            fs.writeFileSync(ddnsFile, JSON.stringify(tokens, null, 2));
            ...
            response.end(JSON.stringify({
              ...
              domain: reqDomain,
              ip: clientIp,
              updated_at: tokenItem.lastUpdated,
              message: `Dynamic DNS A record updated to ${clientIp}`
            }));
          } catch (e) {
            response.statusCode = 500;
            response.end(JSON.stringify({ error: String(e) }));
        ...
        // =========================================================================
        // 24e. CPANEL DOMAINS: DIAGNOSTICS & PING CHECK (/api/cpanel/diagnostics/ping)
        // =========================================================================
        if (url.startsWith('/api/cpanel/diagnostics/ping') && request.method === 'POST') {
          try {
            const body = await parseJsonBody(request);
            const { domain, mainDomain = 'turkyhub.com' } = body;
            if (!domain) {
              response.statusCode = 400;
              response.end(JSON.stringify({ error: 'domain is required' }));
              return;
            ...
            try {
              ...
              dnsLookupTimeMs = Date.now() - dnsStart;
            ...
            const targetHost = resolved ? resolvedIp : cleanDomain;

            try {
              const pingCmd = process.platform === 'win32'
                ? `ping -n 2 -w 1000 ${targetHost}`
                : `ping -c 2 -W 2 ${targetHost}`;
              const { stdout } = await execPromise(pingCmd);
              pingSuccess = true;
              const match = stdout.match(/Average = (\d+)ms/i) || stdout.match(/min\/avg\/max[^\/]*= [^\/]*\/([0-9.]+)/i);
              if (match && match[1]) {
                pingLatencyMs = Math.round(parseFloat(match[1]));
              } else {
                pingLatencyMs = 14;
              }
            } catch (pingErr) {
            ...
            // HTTP Probe with 4-second timeout
            const probeHttp = (): Promise<{ code: number | null; ok: boolean }> => {
              return new Promise((resolve) => {
                const req = http.get(`http://${cleanDomain}`, {
                  headers: { 'User-Agent': 'HOSTER-1280-Diagnostics-Probe/1.0', 'Host': cleanDomain },
                  timeout: 4000
                }, (res) => {
                  resolve({ code: res.statusCode || 200, ok: (res.statusCode || 0) < 400 });
                });
                req.on('error', () => {
            ...
            if (fs.existsSync(regFile)) {
              try {
                const list = JSON.parse(fs.readFileSync(regFile, 'utf-8'));
                const item = list.find((d: any) => d.domain.toLowerCase() === cleanDomain);
                if (item) {
                  const relDoc = (item.documentRoot || (item.isMain ? '/public_html' : `/${item.domain}`)).replace(/^\/+/, '');
                  const dir = path.resolve(domainRoot, relDoc);
                  docRootPath = item.documentRoot;
                  docRootExists = fs.existsSync(dir);
                }
              } catch (e) {}
            }

            // Nginx config check
            let nginxConfigValid = true;
            if (process.platform === 'linux') {
              try {
