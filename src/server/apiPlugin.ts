...
# Generated by Sitechai Cloud Web Server Engine
<VirtualHost *:80>
    ServerName ${domain}
...
# Managed by Sitechai Cloud Web Server Engine
server {
    listen 80;
    listen [::]:80;
    server_name ${domain} www.${domain};
...
export function ensureStandardDomainStructure(domain: string, isFirstInit: boolean = false) {
  ...
  if (isBrandNewDomain || isFirstInit) {
    ...
    if (!fs.existsSync(htaccess)) {
      fs.writeFileSync(
        htaccess,
        `# Sitechai Cloud Server Configuration for ${domain}\nRewriteEngine On\nDirectoryIndex index.html index.php\n`
      );
    }
...
// Sitechai Cloud Web Server Engine - ${finalDomain}
?>
<!DOCTYPE html>
<html lang="en">
<head>
        ...
        if (url.startsWith('/api/cpanel/domains/repair') && request.method === 'POST') {
          try {
            ...
            if (!fs.existsSync(indexPhp) && !fs.existsSync(indexHtml)) {
              ...
              const defaultContent = `<?php
// Sitechai Cloud Web Server Engine - ${target.domain}
?>
<!DOCTYPE html>
<html lang="en">
<head>
