    ...
    <div className="min-h-screen bg-[#f7f9fa] text-slate-800 flex flex-col font-sans select-none">
      ...
      <div className="bg-[#f2f4f7] border-b border-slate-300 px-4 py-2 flex flex-wrap items-center justify-between gap-2 text-xs">
        ...
        <div className="flex items-center gap-2">
          ...
          {/* Restore Default (Requirement #6: Visible strictly and ONLY at DOMAIN ROOT /) */}
          {(currentPath === '/' || currentPath === '') && (
            <button
              onClick={() => setActiveModal('restore_confirm')}
              className="px-2.5 py-1 rounded bg-emerald-50 hover:bg-emerald-100 text-emerald-700 font-bold flex items-center gap-1 border border-emerald-300 shadow-2xs transition-colors"
              title="Recreate missing default hosting directories without touching user content"
            >
              <RotateCcw className="w-3.5 h-3.5 text-emerald-600" />
      ...
      <div className="flex-1 flex min-h-0 overflow-hidden">
        ...
        <aside className="w-64 sm:w-72 bg-white border-r border-slate-200 flex flex-col shrink-0 overflow-y-auto">
          <div className="bg-[#f0f4f8] text-[#004b99] px-3 py-2 font-bold text-xs border-b border-slate-200 flex items-center">
                ...
                {filteredItems.length === 0 ? (
                  <tr>
                    <td colSpan={7} className="py-16 text-center text-slate-400 font-medium text-xs">
                      {isLoading ? 'Loading files from server...' : 'This directory is empty.'}
                    </td>
                  </tr>
                ) : (
          ...
          <div className="bg-white rounded-2xl max-w-sm w-full p-6 shadow-2xl border border-slate-200 text-xs">
            ...
            <div className="space-y-3 mb-6">
              <div className="flex items-center gap-2">
                ...
                />
                <label htmlFor="showHiddenCheck" className="font-semibold text-slate-700 select-none cursor-pointer">
                  Show Hidden Files (dotfiles like .htaccess)
                </label>
              </div>
            </div>
