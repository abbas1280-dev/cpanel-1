...
# ==============================================================================
echo "[+] Configuring Dedicated VirtualHost for HOSTER 1280 Portal (/etc/nginx/sites-available/hoster1280.shop.conf)..."
cat << 'EOF' > /etc/nginx/sites-available/hoster1280.shop.conf
# ==============================================================================
# HOSTER 1280 - Master Control Panel & Client Area Edge Proxy
# ==============================================================================
server {
    listen 80;
    listen [::]:80;
    listen 443 ssl;
    listen [::]:443 ssl;
    server_name hoster1280.shop www.hoster1280.shop cpanel.hoster1280.shop;
    ...
    # HOSTER 1280 Vite Control Panel & API (Port 5173)
    location / {
...
ln -sf /etc/nginx/sites-available/hoster1280.shop.conf /etc/nginx/sites-enabled/hoster1280.shop.conf

echo "[+] Configuring Fallback Default VirtualHost (/etc/nginx/sites-available/default)..."
...
EOF
    ...
    chmod 755 "$DOCROOT"
    if [ "$DOMAIN" = "turkyhub.com" ]; then
        chmod 755 /home/turkyhu1
        chown -R turkyhu1:www-data "$DOCROOT" 2>/dev/null || chown -R www-data:www-data "$DOCROOT" 2>/dev/null || true
    else
...
server {
    ...
    location ~ \.php$ {
        ...
        fastcgi_pass unix:${TENANT_PHP_SOCK};
...
echo ""
echo "================================================================================"
echo "                       VERIFICATION & DIAGNOSTIC AUDIT                          "
echo "================================================================================"
echo -n "1. Local HTTP (hoster1280.shop):       "
curl -s -o /dev/null -w "%{http_code}\n" -H "Host: hoster1280.shop" http://127.0.0.1/ || echo "Failed"

echo -n "2. Local HTTPS (hoster1280.shop):      "
curl -k -s -o /dev/null -w "%{http_code}\n" -H "Host: hoster1280.shop" https://127.0.0.1/ || echo "Failed"

echo -n "3. Local HTTP (turkyhub.com):          "
curl -s -o /dev/null -w "%{http_code}\n" -H "Host: turkyhub.com" http://127.0.0.1/ || echo "Failed"

echo -n "4. Local HTTPS (turkyhub.com):         "
curl -k -s -o /dev/null -w "%{http_code}\n" -H "Host: turkyhub.com" https://127.0.0.1/ || echo "Failed"

echo -n "5. Bind9 DNS (hoster1280.shop A):      "
dig @127.0.0.1 hoster1280.shop A +short 2>/dev/null || echo "Failed"

echo -n "6. Bind9 DNS (turkyhub.com A):         "
dig @127.0.0.1 turkyhub.com A +short 2>/dev/null || echo "Failed"

echo -n "7. Bind9 DNS (ns1.hoster1280.shop A):  "
dig @127.0.0.1 ns1.hoster1280.shop A +short 2>/dev/null || echo "Failed"
echo "================================================================================"
